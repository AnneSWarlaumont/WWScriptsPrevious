...
calcOddsRatio <- function(matrix4,alpha=0.5,referencerow=2,quiet=FALSE) {
	...
	for (i in 1:numrow) {
		...
		if (i != referencerow) {
			...
			#calculate the odds ratio
			oddsRatio <- (probInfantVocGivenNotToChild*probNInfantVocGivenNNotToChild)/(probNInfantVocGivenNotToChild*probNInfantVocGivenNNotToChild)
...
calcOddsRatio <- function(matrix5,alpha=0.05,referencerow=2,quiet=FALSE){
	...
	for (i in 1:numrow){
		...
		if(i != referencerow) {
			...
			probToChildGiveReflex <- ToChildProto/totProto
			probToChildGivenReflex <- ToChildReflex/totReflex
			probNToChildGiveReflex <-NToChildProto/totProto
			probNToChildGivenReflex <-NToChildReflex/totReflex
			
			#calculate the odds ratio
			oddsRatio <- (probToChildGiveReflex*probNToChildGivenReflex)/(probNToChildGiveReflex*probToChildGivenReflex)
			if(quiet == FALSE) {
				print(paste("category =", rowname, ", odds ratio =",oddsRatio))
			}
...
calcOddsRatio <- function(matrix7,alpha=0.05,referencerow=2,quiet=FALSE){
	...
	for (i in 1:numrow){
		...
		if(i != referencerow) {
			...
			probNotToChildGiveReflex <- NotToChildProto/totProto
			probNotToChildGivenReflex <- NotToChildReflex/totReflex
			probNNotToChildGiveReflex <-NNotToChildProto/totProto
			probNNotToChildGivenReflex <-NNotToChildReflex/totReflex
			
			#calculate the odds ratio
			oddsRatio <- (probNotToChildGiveReflex*probNNotToChildGivenReflex)/(probNNotToChildGiveReflex*probNotToChildGivenReflex)
			if(quiet == FALSE) {
				print(paste("category =", rowname, ", odds ratio =",oddsRatio))
			}
